...
GameState.prototype = new function() {
    ...
    //                      on the outcome.
    this.calculateStolenBase = function (sbapct, sbpct) {
        if (this.bases[2] == 0) {
    ...
    this.handleRunners = function (atBatOutcome) {
        var runners = '';
        for (i = 1; i <= 3; i++) {
    ...
    //              correct base.
    this.handleHitter = function (atBatOutcome, sbapct, sbpct) {
        switch (atBatOutcome) {
            case 'si':
    ...
    this.processOutcome = function (atBatOutcome, sbapct, sbpct) {
    ...
    this.resolveConflict = function (origin, dest) { 
